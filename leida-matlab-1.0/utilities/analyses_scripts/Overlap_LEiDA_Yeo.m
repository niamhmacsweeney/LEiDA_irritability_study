function [cc_V_yeo7,p_V_yeo7] = Overlap_LEiDA_Yeo(parcellation,n_areas,centroids,rangeK, index100areas)
%
...
%USER (Niamh) Edit to adapt for Yeo Overlap
% First load the mask of the chosen parcellation in MNI space 2mm
V_Parcels = struct2array(load(['ParcelsMNI2mm'],['V_' parcellation]));
V_new_Parcels=zeros(size(V_Parcels));
for n=index100areas
    ind_n= V_Parcels(:)==n ;
    V_new_Parcels(ind_n)=find(index100areas==n);
end
V_Parcels=V_new_Parcels;


%original script
%V_Parcels = struct2array(load(['ParcelsMNI2mm'],['V_' parcellation]));
%V_new_Parcels((V_Parcels > n_areas)) = 0;
