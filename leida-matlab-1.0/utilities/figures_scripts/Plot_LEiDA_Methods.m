...
addpath  /Users/joana/Documents/Work/Mushrooms
%
...
load('/Users/joana/Documents/Work/LEiDA general/LEiDA/example_BOLD.mat','tc_aal')
signal = tc_aal{Subject,Condition};
[N_areas, Tmax]=size(signal);
clear tc_aal 

% USE IF YOUR DATA IS NOT IN SYMMETRIC COORDINATES
...
Theta=angle(Complex_BOLD(seed,:));
Amp=abs(Complex_BOLD(seed,:));

% Panel A - FIGURE ILLUSTRTING BOLD HILBERT PHASE
figure
plot3(0:TR:(length(Complex_BOLD)-1)*TR,imag(Complex_BOLD(seed,:)),real(Complex_BOLD(seed,:)),'k','LineWidth',2);
xlabel('Time (seconds)')
ylabel('Imag')
zlabel('Real')
grid on
...
hold on
plot3(0:TR:(length(Theta)-1)*TR,3*ones(size(Theta)),signal_filt(seed,:),'b');
plot3(0:TR:(length(Theta)-1)*TR,3*ones(size(Theta)),signal(seed,:),'g');
...
% LOAD ANY SEQUENCE OF EIGENVECTORS
